...
const LampScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        console.log("đây là đèn: ", device.nameDevice);
        console.log(valueDevices.shared[device.idDevice]);
        console.log("công tắc realtime: ", isRealtimeModeOn, ", công tắc đèn: ", isLightOn);
    }, [])


    useEffect(() => {
        // console.log("firsttime into isRealtimeModeOn: ", firsttime);
        ...
        const fetchData = async () => {
            ...
            if (isRealtimeModeOn === true) {
                const intervalId = setInterval(() => {

                    const currentTime = moment();
                    const isDaytime = currentTime.hours() >= 6 && currentTime.hours() < 18;
                    // light is off during daytime, on otherwise
                    // if (!isDaytime) {
                    //     setIsLightOn(!isDaytime);
                    // }
                    console.log("chay xet 1s cua real time: ", isRealtimeModeOn);
                    setIsLightOn(!isDaytime);

                    console.log("isRealtimeModeOn: ", isRealtimeModeOn);
                    if (isRealtimeModeOn === false) {
                        console.log("isRealtimeModeOn: x", isRealtimeModeOn);
    ...
    const toggleRealtimeMode = async () => {
        ...
        console.log("hiện tại công tắc real time: ", isRealtimeModeOn);
    }
    ...
    useEffect(() => {
        // console.log("firsttime into isLightOn: ", firsttime);

        console.log('công tắc đèn: ', isLightOn);
        let isLightOnPost = "";
        if (isLightOn === true) isLightOnPost = "1";
        else isLightOnPost = "0";
        // console.log("đến lúc gửi post để thay đổi giá trị đèn, với device.idDevice: ", device.idDevice, ", và isLightOnPost: ", isLightOnPost);
        if (isLightOn) setValue(50);
        else setValue(0);
        const toggleLightAPI = async (idDevice, isLightOnPost) => {
            try {
        ...
        if (firsttime === false) {
            toggleLightAPI(device.idDevice, isLightOnPost)
        }
